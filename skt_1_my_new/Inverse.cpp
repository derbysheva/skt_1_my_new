...
#include "Inverse.h"
#include <iostream>
Inverse:: Inverse (Area& cur_area, Receiver& cur_rec)
{
	...
	inv_area = cur_area;
	inv_rec = cur_rec;
	K = inv_area.cells.size();// количество кубсонов
...
void Inverse:: solve(vector<vector<double>>& A_c) 
{
	gauss(A_c, b, b.size(), p);
	//распишем р по кубсонам
...
double Inverse:: find_best_alpha() 
{
...
void Inverse:: add_alpha(double alp, vector<vector<double>>& A_c)
{
	A_c = A;
	for (int q = 0; q < A.size(); q++)
		A_c[q][q] += alp;
...
vector<vector3> Inverse:: find_best_gamma()
{
...
void Inverse:: add_gamma(vector<vector3> gam, vector<vector<double>>& A_c) 
{
	//добавим в матрицу
	int m;
	//по всем кубсонам
