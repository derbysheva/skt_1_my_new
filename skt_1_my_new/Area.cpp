...
vector3 Area:: get_B(vector3 point) 
{
	//общее ¬ - сумма ¬ по всем кубсонам
...
void Area:: generate(const string& filename) 
{
	...
	int nx, ny, nz;
	fin >> nx >> ny >> nz;// количество кубиков
	...
	for (int k = 0; k < nz; k++) {
		for (int j = 0; j < ny; j++) {
			for (int i = 0; i < nx; i++) {
				...
				t.init(); //здесь рассчитываетс€ объем и центр куба

				//вычислим (глобальный) номер текущего куба
				...
				t.p = vector3(0, 0, 0);
				cells.push_back(t);//все разбиени€ на кубы
			}
		}
	}
	fin.close();
}
void Area:: generate_anoms(const string& anomal) {
	...
	for (int i = 0; i < n_anom; i++) {
		...
		//выбрать кубы, вход€щие в область текущей iтой аномалии и задать им р
		// кубсон полностью входит в область аномалии
...
void Area:: direct (Receiver& cur_rec, vector<vector3>& B_cur) 
{
